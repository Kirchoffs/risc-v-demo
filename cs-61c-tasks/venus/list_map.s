...
main:
    ...
    add a0, s0, x0           # a0 = s0 which is the head of node list
    jal ra, print_list
    jal ra, print_newline
...
map:
    ...
    ### PRACTICE ###
    addi sp, sp, -12
    sw ra, 8(sp) # return address because of recursion
    sw s0, 4(sp) # head of linkedlist
    sw s1, 0(sp) # saved register

    # a0 points to current node
    # a1 points to the function
